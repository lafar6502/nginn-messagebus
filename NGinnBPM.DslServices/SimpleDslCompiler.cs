...
namespace NGinnBPM.DSLServices
{
    ...
    public class SimpleBaseClassDslCompiler<T>
    {
        ...
        protected void FinishAssemblyCreation(CompilerContext cc)
        {
            if (cc.GeneratedAssembly != null) return;
            var pb = cc.GeneratedPEBuilder;
            if (pb == null) throw new Exception("No PE builder, can't generate the assembly");
            var bb = new System.Reflection.Metadata.BlobBuilder();
            pb.Serialize(bb);
            
            byte[] asmData = bb.ToArray();
            var asm = Assembly.Load(asmData);
            cc.GeneratedAssembly = asm;
        }
